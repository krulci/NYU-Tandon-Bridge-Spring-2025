...
void RBT<T>::singleCR(RBTNode<T> *&point) {
    ...
    // Parent becomes new root of this subtree
    parent->right = grandparent;
    parent->parent = grandparent->parent;

    // Update parent's parent to point to parent
...
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) {
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        while (curr_node != root && curr_node->parent->color == RED)
        {
            ...
            // Always swap color of parent and grand parent to comply with rule 3.
            swapColor(parent);
            swapColor(grandparent);

            // Uncle is red: flip colors
